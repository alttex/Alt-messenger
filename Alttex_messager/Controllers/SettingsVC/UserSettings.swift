...
//  Alttex_messager
//
//  Created by Vlad Kovryzhenko on 3/11/18.
...
//        }
    ...
    func displayFirebaseUserInformations(){
        if let id = Auth.auth().currentUser?.uid {
            User.info(forUserID: id, completion: {[weak weakSelf = self] (user) in
                DispatchQueue.main.async {
                    ...
                    self.userInfo.text = user.name
                    weakSelf?.nameImageView.image = user.profilePic
                    weakSelf = nil
                }
            })
        }
       ...
       // self.currentUser.profilePic.image = notification.userInfo?["pic"] as? UIImage
...
//        let oldProfilePictureReference = firebaseStorageReference.child("user").child(currentUser.id)
